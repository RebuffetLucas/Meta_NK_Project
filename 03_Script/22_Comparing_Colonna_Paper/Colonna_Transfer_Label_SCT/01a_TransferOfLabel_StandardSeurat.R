...
PBMC_Blood_All = readRDS("/mnt/DOSI/EVLAB/BIOINFO/BIOINFO_PROJECT/Meta_NK5/05_Output/22_Comparing_Colonna_paper/Colonna_Transfer_Label_V1/Seurat_Objects/NKcellsfrom4tissues.rds")

PBMC_Blood_All@assays[["HTO"]]= NULL
PBMC_Blood_All@assays[["SCT"]]= NULL
...
colnames(table_samples) = c("Tissue" , "Tissue2", "Number of Cells")
...
#Merge data
data_all =  merge(PBMC_Blood_subset, y = PBMC_Meta_NK_V2, add.cell.ids = c("Colonna", "Meta_V2"))
...
metadata_Tang$Project="Colonna"
metadata_Tang$nkg2c="unknown"
rownames(metadata_Tang) = paste0("Colonna_",rownames(metadata_Tang))
...
for (i in 1:length(data_all_list)) {
  data_all_list[[i]] = SCTransform(data_all_list[[i]], verbose = FALSE)
}


#Integration
data_features= SelectIntegrationFeatures(object.list = data_all_list, nfeatures = 3000)
data_all_list <- PrepSCTIntegration(object.list = data_all_list, anchor.features = data_features)

reference_dataset = which(names(data_all_list) %in% REFERENCE_FOR_INTEGRATION)

data.anchors <- FindIntegrationAnchors(object.list = data_all_list, normalization.method = "SCT", 
                                       anchor.features = data_features, reference = reference_dataset)
...
#Integration.anchors= FindIntegrationAnchors(object.list = reference.list, dims= 1:30, normalization.method = "SCT")
#data_integrated =  IntegrateData(anchorset = Integration.anchors, dims = 1:30, normalization.method = "SCT")
...
#Iterative Mapping
#List_query = data_all_list[QUERY_DATASETS]


#for (i in 1:length(List_query)){
#  data.anchors <- FindTransferAnchors(reference = data_integrated, query = List_query[[i]], dims = 1:30, normalization.method = "SCT")
#  predictions <- TransferData(anchorset = data.anchors, refdata = data_integrated$FirstClust ,dims = 1:30)
#  List_query[[i]]= AddMetaData(List_query[[i]], metadata = predictions)
#}

#saveRDS(List_query, file=paste0(PATH_ANALYSIS_OUTPUT,"/List_query_IterativelyGenerated_SCTNorm.rds"))
...
#Test to map them iteratively:

#List_query = data_all_list[QUERY_DATASETS]

#for (i in 1:length(List_query)){
#  data.anchors <- FindTransferAnchors(reference = data_integrated, query = List_query[[i]],  dims = 1:30)
#  predictions <- TransferData(anchorset = data.anchors, refdata = data_integrated$FirstClust ,dims = 1:30)
#  List_query[[i]]= AddMetaData(List_query[[i]], metadata = predictions)
#}
