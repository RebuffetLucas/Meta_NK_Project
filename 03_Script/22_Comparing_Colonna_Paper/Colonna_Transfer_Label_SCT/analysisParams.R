...
SUB_ANALYSIS_STEP_NAME= "Colonna_Transfer_Label_SCT"
...
LITERAL_TITLE = "Reference based SCT Process for label Seurat_Transfer applied on all Colonna's samples, tissue NK cells from Colonna, here we use 3 pops of NK cells (NK1 , NK2,  NK3 since NKint are considered as NK1 cells)
                  We look at blood and healthy tissues Colonna's data"
...
DATA_SPLIT =  "meta_patientID"  #Data to split by for normalization and label transfer procedure #BatchCor = samples for MetaNK and orig.ident for Tang

#Use composed data split
DO_USE_COMPOSED_DATA_SPLIT_FOR_TANG = FALSE
...
DO_USE_CATEGORY_TRESHOLD = FALSE
MINIMAL_NUMBER_BATCH_COR_CATEGORY = 100


DO_SAVE_QUERYandREF = TRUE
QUERY_RDS_NAME = "data.query_all_cells_Healthy_Tissues_Colonna_SCTnorm_AllMAppedatONCE.rds"
DATA_QUERY_SAVE_PATH = file.path(PATH_ANALYSIS_OUTPUT, QUERY_RDS_NAME ) 

REF_RDS_NAME =  "data.ref_MetaNK_all_cells_SCTNORM.rds"
