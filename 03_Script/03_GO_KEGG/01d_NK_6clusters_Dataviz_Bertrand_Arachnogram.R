...
GO_to_plot = c("regulation of actin filament-based process"  ,     "immune response-activating cell surface receptor signaling pathway" ,
                            "cell killing"     ,   "tricarboxylic acid cycle" ,               
                            "positive regulation of cytokine production",    "regulation of tumor necrosis factor superfamily cytokine production" , 
                            "lymphocyte differentiation"        ,           "positive regulation of leukocyte cell-cell adhesion"   )             
...
p = ggplot(MT2, aes(x=as.factor(Description), y=GeneRatio, fill=Cluster)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar
  ...
  annotate("text", x = 0.45, y = c(2, 4,6,8), label = c("2", "4", "6", "8") , color="grey20", size=3 , angle=0, fontface="bold", hjust=1) +
  annotate("text", x = 0.45, y = 8.5, label = "-log10(p.adj)" , color="grey20", size=3 , angle=0, fontface="bold", hjust=1) +
  ...
  theme(
    ...
    ) + geom_hline(yintercept = -log10(0.05),linetype="dashed")+geom_hline(yintercept = c(2,4,6,8),linetype="dashed",color = "grey20",alpha=0.3)+
  geom_vline(xintercept = c(0.5,1.5,2.5,3.5,4.5,5.5,6.5,7.5),linetype="dashed",color = "grey20",alpha=0.3)+
  coord_polar(start=0)
...
p2 = ggplot(MT2, aes(x=as.factor(Description), y=GeneRatio, fill=Cluster)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar
  ...
  # Add text showing the value of each 100/75/50/25 lines
  annotate("text", x = 0.45, y = c(2, 4,6,8), label = c("2", "4", "6", "8") , color="grey20", size=4 , angle=0, fontface="bold", hjust=1) +
...
png(file="/mnt/DOSI/SUEVLAB/BIOINFO/BIOINFO_PROJECT/Meta_NK5/05_Output/03_GO_KEGG/Figures_V3/6clusters_WithLegend.png", width = 20, height =20, units= "cm" ,  res=600 )
p
dev.off()


png(file="/mnt/DOSI/SUEVLAB/BIOINFO/BIOINFO_PROJECT/Meta_NK5/05_Output/03_GO_KEGG/Figures_V3/6clusters_NoLegend.png", width = 20, height =20, units= "cm" ,  res=600 )
p2
dev.off()
