...
REFERENCE_DATASETS = c( "P3_0", "P2_0",  "P1_0" ,"P4_0" ,"P7_0" ,"P6_0" , "P8_0", "P5_0")



#### Dealing with RNA first ####

#Re processing SCT on the subset
Seurat_NK2= SCTransform(Seurat_NK, assay= "SCT", new.assay.name = "SCT" , return.only.var.genes = FALSE)
...
#Find anchors
index_ref =  which(names(Seurat_NK_List) %in% REFERENCE_DATASETS ) 
immune.anchors <- FindIntegrationAnchors(object.list = Seurat_NK_List, normalization.method = "SCT", reference = index_ref ,
...
#Find anchors
index_ref =  which(names(Seurat_NK_List) %in% REFERENCE_DATASETS ) 
immune.anchors2 <- FindIntegrationAnchors(object.list = Seurat_NK_List,   assay= NULL, normalization.method = "LogNormalize", reference= index_ref,
...
Seurat_NK_Final = ScaleData(Seurat_NK_Final ,  do.scale = DATA_SCALE, do.center = DATA_CENTER)
...
#Saving
saveRDS(Seurat_NK_Final, "/mnt/DOSI/SUEVLAB/BIOINFO/BIOINFO_PROJECT/Meta_NK5/05_Output/12_CITEseq_Analysis/Final_After_Satija_Procedure_D0ref_D3_D7.rds")
