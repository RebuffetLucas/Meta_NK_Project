...
DotPlot(PBMC, features = paste0(names(MONITORED_Markers),"1"), group.by = "celltype", cols = "Spectral") + theme(axis.text.x = element_text(angle = 90))
VlnPlot(PBMC, features = paste0(names(MONITORED_Markers),"1"), group.by = "celltype", pt.size= 0)    & theme(axis.text.x = element_text(angle = 90))
