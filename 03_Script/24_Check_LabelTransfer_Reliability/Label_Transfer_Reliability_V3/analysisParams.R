...
SUB_ANALYSIS_STEP_NAME= "Label_Transfer_Reliability_V3"
...
LITERAL_TITLE = "Here we try to assess to what extent the label classic label transfer of Seurat is reliable we split MetaNK in 80% ref vs 20 % query and use the 3 pops of NK cells classification (NK1 , NK2,  NK3)
                  (NKint are considered as NK1) We train it by taking 20% of cells.       For Normalization and FindVariableFeatures we nsplit by split = paste0 (query/Ref , orig.ident )"
...
SUBSET_DOWNSAMPLE_QUERY = TRUE #Whether downsample Query
PERCENT_DOWNSAMPLE = 0.2 #Number of cells to downasample randomly
