...
cat("### Realiability treshold")

#subset only the true predictions

data.query$True_Predict = data.query$predicted.id == data.query$FirstClust

table(data.query$True_Predict)

data.query_TRUEpred_Only = subset(data.query, subset = True_Predict == TRUE )
data.query_FALSEpred_Only = subset(data.query, subset = True_Predict == FALSE )

p20 = VlnPlot(data.query , features= "prediction.score.max", group.by = "predicted.id" , cols = palette2, pt.size = 0, y.max = 1.1) & stat_summary(fun.data=data_summary,color="black") & ggtitle("score max for all query")

cat("\n\n")
print(p20)
cat("\n\n")

p22 = VlnPlot(data.query_TRUEpred_Only , features= "prediction.score.max", group.by = "predicted.id" , cols = palette2, pt.size = 0, y.max = 1.1) & stat_summary(fun.data=data_summary,color="black") & ggtitle("score max for query with right prediction")
...
p24 = VlnPlot(data.query_FALSEpred_Only , features= "prediction.score.max", group.by = "predicted.id" , cols = palette2, pt.size = 0, y.max = 1.1) & stat_summary(fun.data=data_summary,color="black") & ggtitle("score max for query with wrong prediction")
...
p21 = VlnPlot(data.query , features= "prediction.score.max", group.by = "predicted.id" , cols = palette2, pt.size = 0, y.max = 1.1, split.by = "True_Predict") & ggtitle("score max for all query")
