...
SUB_ANALYSIS_STEP_NAME= "Label_Transfer_Reliability_V2"
...
LITERAL_TITLE = "Here we try to assess to what extent the label classic label transfer of Seurat is reliable we split MetaNK in 80% ref vs 20 % query and use the 4 pops of NK cells classification (NK1 , Nkint, NK2,  NK3) We train it by taking 20% of cells in each subset for Normalization and FindVariable Features we need to do Query and Ref together otherwise samples are too small to run the functions "
